...
<body class="bg-gray-100">
  <div class="flex flex-col md:flex-row h-screen">
    ...
    <main class="flex-1 p-6 h-max-[100vh] overflow-y-auto">
      <section class="mb-6">
        <h1 class="text-3xl font-bold">Ajouté un nouveau Nationalité</h1>
      </section>
      <section id="userForm" class="mb-8">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-2xl mx-auto">
        <form id="player-form" method="POST">
        <div class="mb-4">
            <label for="Nationalité" class="block text-sm font-medium text-gray-700">Nationalité</label>
            <input type="text" id="Nationalité" name="Nationalité" placeholder="Entrez Nationalité" min="1" max="100" required class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
        </div>
        <div class="mb-4">
            <button type="submit" class="w-full px-4 py-2 bg-indigo-500 text-white rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">add</button>
        </div>
        </form>
        </div>
        </section>
